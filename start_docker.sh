...
while getopts ":hp:c:i:n:t:b:zr" opt; do
case ${opt} in
    p )
        file_path=$OPTARG
    ;;
...
imagetagstring=`echo $imagetag|sed -e 's/:/_/g'`
...
if [ $start_container == 1 ]; then
    ...
    if [ $d_or_r = "docker" ];then
        #docker run -v host_path:container_path
        docker run --net=$network -d --rm --name $container -v $file_path:$file_path $imagetag $shell -c "while true; do sleep 5; done"
    elif [ $d_or_r = "rocker" ];then
